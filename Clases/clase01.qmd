...
::: {.column width="50%"}
-   Enfrentamientos:
    -   A vs B: A gana (75% vs 25%)
    -   A vs C: A gana (55% vs 45%)
    -   A vs D: D gana (35% vs 65%)
    -   B vs C: B gana (80% vs 20%)
    -   B vs D: B gana (60% vs 40%)
    -   C vs D: C gana (80% vs 20%)
-   **Resultado**: Tenemos un empate **A y B** Ganan rondas
:::
:::::
...
-   **Ejemplo**:
    ...
    -   Preferencias de los votantes:
        ...
        -   20%: D \> C \> B \> A
    -   **Ronda 1**:
        -   D tiene menos votos, es eliminado.
:::

::: {.column width="50%"}
-   **Ronda 2**:
    -   Los votos de D se redistribuyen a C (40% en total).
    -   Totales: A (30%), B (25%), C (45%).
    -   B tiene menos votos, es eliminado.
-   **Ronda 3**:
    -   Los votos de B se redistribuyen a C.
    -   Totales: A (30%), C (70%).
-   **Resultado**: C es el ganador tras las eliminaciones.
:::
:::::
...
-   **Ejemplo**:
    ...
    -   Primera vuelta:
        ...
        -   D: 13%
    -   **Segunda vuelta**:
        -   A vs B: B gana con 55% frente a A (45%).
    -   **Resultado**: B gana la elección en la segunda vuelta.
...
-   **Ejemplo**:
    ...
    -   Preferencias de los votantes:
        -   30%: A \> B \> C \> D\
            $(A:3 \times 30\%,\; B:2 \times 30\%,\; C:1 \times 30\%)$
:::

::: {.column width="50%"}
-   Preferencias de los votantes:
    -   25%: B \> C \> D \> A\
        $(B:3 \times 25\%,\; C:2 \times 25\%,\; D:1 \times 25\%)$
    -   25%: C \> D \> A \> B\
        $(C:3 \times 25\%,\; D:2 \times 25\%,\; A:1 \times 25\%)$
    -   20%: D \> C \> B \> A\
        $(D:3 \times 20\%,\; C:2 \times 20\%,\; B:1 \times 20\%)$
:::
:::::
...
-   **Resultado**:
    -   A: 90 + 25 = 115 puntos\
    -   B: 60 + 75 + 20 = 155 puntos\
    -   C: 30 + 50 + 75 + 40 = 195 puntos\
    -   D: 25 + 50 + 60 = 135 puntos\
    -   **Ganador**: C es el ganador según el conteo de Borda.
...
ggplot(data.frame(x = c(0, 10)), aes(x = x)) +
  ...
  geom_label(aes(x = 2, y = IC_6(2)), 
             label = "u == sqrt(32)", parse = TRUE, 
             color = "#20A387FF", size = 4) +
  geom_label(aes(x = 2, y = BC(2)), 
             label = "BC", 
             color = "red", size = 6) +
  ...
  scale_y_continuous(breaks = seq(0, 20, 2), limits = c(0, 20), expand = expansion(mult = c(0, 0.1))) +
  labs(x = "x", y = "y", 
       caption = expression(paste(u(x, y) == sqrt(xy), ", ", m == "$20", ", ", p[x] == "$1", ", ", p[y] == "$2"))) +
  theme_classic(base_size = 20)
```
:::
::::
...
ggplot(data.frame(x = c(0, 10)), aes(x = x)) +
  ...
  geom_label(aes(x = 2, y = BC(2)), 
             label = "BC", 
             color = "red", size = 6) +
  geom_label(aes(x = 3.5, y = IC_7(3.5)), 
             label = "u == sqrt(50)", parse = TRUE, 
             color = "#3CBB75FF", size = 4) +
  # Flecha morada
  annotate("segment", x = 16, xend = 10, y = 2, yend = 5, 
           colour = "purple", size = 2, alpha = 1, 
           arrow = arrow(length = unit(0.5, "cm"), ends = "last", type = "closed")) +
  # Puntos y etiquetas A y C
  geom_point(data = subset(points, letter %in% c("A", "C")), 
             aes(x = x, y = y), size = 3) +
  geom_text_repel(data = subset(points, letter %in% c("A", "C")), 
                  aes(x = x, y = y, label = letter), size = 6, seed = 2) +
  # Línea discontinua
  geom_segment(aes(x = 0, xend = 20, y = 4, yend = 1.5), 
  ...
  # Leyenda del gráfico
  labs(x = "x", y = "y", 
       caption = expression(paste(u(x, y) == sqrt(xy), ", ", 
                                  m == "$20", ", ", 
                                  p[x] == "$1", ", ", 
                                  p[y] == "$2"))) +
  theme_classic(base_size = 20)
